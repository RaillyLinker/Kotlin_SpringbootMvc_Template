...
object StompConst {
     ...
     */
    val BROKER_DESTINATION_PREFIXES = arrayOf("/topic", "/queue")

    // WebSocket 으로 전송되는 메시지의 최대 크기 설정
    const val MESSAGE_SIZE_LIMIT = 160 * 64 * 1024

    // 메시지 전송에 대한 시간 제한 설정
    const val SEND_TIME_LIMIT = 100 * 10000

    // 송신 버퍼의 크기 제한을 설정
    const val SEND_BUFFER_SIZE_LIMIT = 3 * 512 * 1024
}