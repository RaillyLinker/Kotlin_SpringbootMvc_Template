CREATE TABLE `payment_detail_bank_transfer` (
	...
	`payment_uid`	BIGINT	NOT NULL	COMMENT '결제 정보 행 고유키',
...
CREATE TABLE `payment_detail_toss_payments` (
	`uid`	BIGINT	NOT NULL	COMMENT '행 고유키',
	`row_create_date`	DATETIME(3)	NOT NULL	COMMENT '행 생성일',
	`row_update_date`	DATETIME(3)	NOT NULL	COMMENT '행 수정일',
	`row_delete_date_str`	VARCHAR(50)	NOT NULL	DEFAULT /	COMMENT '행 삭제일(yyyy_MM_dd_T_HH_mm_ss_SSS_z, 삭제되지 않았다면 /)',
...
CREATE TABLE `payment` (
	...
	`row_update_date`	DATETIME(3)	NOT NULL	COMMENT '행 수정일',
	`row_delete_date_str`	VARCHAR(50)	NOT NULL	DEFAULT /	COMMENT '행 삭제일(yyyy_MM_dd_T_HH_mm_ss_SSS_z, 삭제되지 않았다면 /)',
	`payment_code`	VARCHAR(80)	NOT NULL	COMMENT '결제 코드입니다. 외부 모듈에서 결제를 의뢰할 때에 구분을 위해 입력하는 정보로, {모듈 고유값}_{모듈 내 고유값} 으로 이루어집니다.',
...
ALTER TABLE `payment_detail_bank_transfer` ADD CONSTRAINT `PK_PAYMENT_DETAIL_BANK_TRANSFER` PRIMARY KEY (
	`uid`
);

ALTER TABLE `payment_refund` ADD CONSTRAINT `PK_PAYMENT_REFUND` PRIMARY KEY (
	`uid`
);

ALTER TABLE `payment_detail_toss_payments` ADD CONSTRAINT `PK_PAYMENT_DETAIL_TOSS_PAYMENTS` PRIMARY KEY (
	`uid`
);
